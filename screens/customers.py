...
#         left_eye = order["items"][1] if len(order["items"]) > 1 else None
#
#         # כרטיסים קטנים יותר לעיניים
#         def mini_eye_card(title, color, eye):
#             return ft.Card(
#                 content=ft.Container(
#                     content=ft.Column([
#                         ft.Text(title, weight=ft.FontWeight.BOLD, size=12, color=color),
#                         ft.Text(f"מוצר: {eye['product_name']}" if eye else "אין", size=11),
#                         ft.Text(f"מידה: {eye['size']}" if eye else "", size=11),
#                         ft.Text(f"כמות: {eye['quantity']}" if eye else "", size=11)
#                     ], spacing=2),
#                     padding=5,
#                     bgcolor="#f5f5f5",
#                     border_radius=6,
...
#                 content=ft.Column([
#                     ft.Row([
#                         ft.Text(f"{cust['name']} – {order_date} {order_time}", weight=ft.FontWeight.BOLD, size=13),
#                         ft.Text(status_map.get(order['status'], order['status']), size=12, color="grey")
#                     ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
#                     ft.Row([right_card, left_card], spacing=8),
#                     ft.Text(f"סה\"כ: {total:.2f} ₪", size=12, weight=ft.FontWeight.BOLD, color="red"),
#                     ft.Row(buttons, alignment=ft.MainAxisAlignment.END, spacing=5)
#                 ], spacing=4),
#                 padding=8
#             ),
#             elevation=1,
