...
#
#
# def InventoryScreen(page, current_user, go_back, save_fn_with_sizes, save_fn_without_sizes):
#     page.title = "מסך מלאי"
#
#     content = []
#     categories = [1, 2, 3, 4]
...
#             table_rows = []
#
#             # נשמור את ה-TextFields במבנה נגיש
#             text_fields = {}
#
#             for size in sizes:
#                 row = [ft.Text(size)]
...
#                     row.append(tf)
#                     # שומרים לפי גודל+מוצר
#                     text_fields[(size, product['id'])] = tf
...
#                         product_name = next(p['name'] for p in products if p['id'] == pid)
#                         items.append({
#                             "product_name": product_name,
#                             "size": size,
#                             "count": int(tf.value)
#                         })
#                 save_fn_with_sizes(items)
#
#             content.append(
...
#                     tf
#                 ], spacing=10))
#                 text_fields[product['id']] = tf
#
#             content.append(ft.Column(controls=rows, spacing=5))
#
#             def save_handler(e, cid=cat_id):
#                 items = []
#                 for pid, tf in text_fields.items():
#                     if tf.value and tf.value.isdigit():
#                         items.append({
#                             "product_id": pid,
#                             "count": int(tf.value)
#                         })
#                 save_fn_without_sizes( items)
#
#             content.append(
#                 ft.ElevatedButton("שמור", on_click=save_handler)
#             )
#
#     # כפתור חזרה
#     back_button = ft.ElevatedButton("⬅ חזרה", on_click=go_back)
